...
public class JSONConverter {
    ...
    public static final Gson gson = new GsonBuilder().setDateFormat(SanctionEntity.dateFormat.toPattern())
    ...
    private static class RequestDeserializer implements JsonDeserializer<Request> {
        ...
        private MusicInfos deserializeMusicInfos(JsonObject JSONinfos) {
            ...
            try {
                artist = JSONinfos.get(MusicInfos.ALBUM_NAME).getAsString();
            } catch (ClassCastException | IllegalStateException ignored) { }
            ...
            try {
                proposedBy = JSONinfos.get(MusicInfos.ALBUM_IMAGE_URL).getAsString();
            } catch (ClassCastException | IllegalStateException ignored) { }
            ...
            return new MusicInfos(title, artist, authors, proposedBy, duration, totalVotes, posVotes, size, playedPart);
